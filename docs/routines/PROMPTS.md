...
7. Mark the item done in STATUS.md (move to "## Done", add today's date). Keep the ## Done list TIGHT — no blank lines between `- [x]` bullets — and preserve the single blank line before the closing `---` divider that follows the section (never let a bullet butt directly against `---`, which Markdown mis-renders as a heading). Match the existing tight formatting of the ## Next and ## Never lists.
