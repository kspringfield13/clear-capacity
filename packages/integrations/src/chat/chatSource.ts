...
export function createChatExportSource(): ChatSource {
  return {
    ...
    async sync(weekId, input = "") {
      // An empty/whitespace export is "no data", not malformed JSON —
      // `parseChatExport` tolerates it and yields no records (mirrors how the
      // calendar source's empty `.ics` default yields no events).
