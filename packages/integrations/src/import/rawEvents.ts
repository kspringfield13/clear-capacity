...
function importOne(
  ...
  const capacityPct =
    typeof input.capacity_pct === "number" && Number.isFinite(input.capacity_pct)
      ? Math.max(0.25, Math.round(input.capacity_pct))
 ...
 * string of either (throws `SyntaxError` on malformed JSON, matching
 * `JSON.parse`). Records that lack the required fields, carry an unknown
...
export function importRawEvents(
  ...
  const records = coercePayload(payload);
  ...
  return { events: [...events.values()], work_blocks, skipped };
}
