...
- **Theme is not persisted across reloads (bug)**: because `App.tsx` clobbers the saved theme on mount (above), a user's dark-mode choice reverts to light on reload. Tracked in STATUS.md ## Next. Until fixed, expect a light→dark flash and lost preference on web and Tauri.
