...
- [x] **Audit log shows raw enum/ISO correction values while Corrections screen humanizes them** — added `humanizeCorrectionValue` to the import from `lib/format.ts` in `App.tsx`; changed the audit summary at line 514 from raw `old_value -> new_value` to `humanizeCorrectionValue(field, old_value) → humanizeCorrectionValue(field, new_value)`, so the Audit log now matches the humanized display in CorrectionsScreen. 2026-06-25
