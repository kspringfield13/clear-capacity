...
### UI & UX Polish
- [ ] **Ledger work blocks sit below the fold under the heatmap** — on the Activity Ledger screen (`components/ledger/LedgerScreen.tsx`) the Current-block card, `ActivityCapturePanel`, and the full 7-day `ActivityHeatmap` consume the entire fixed-height `.screen` (`height:100%; overflow:hidden`), so the `.ledger-list` — the screen's primary content — shows only one clipped block at 1280×900 and nothing at all at 420px before its internal scroll. Make `ActivityHeatmap.tsx` a collapsible `<details>` disclosure (reuse the `<details className="evidence">` pattern already in `BlockCard.tsx:153`) and/or trim its row height in `styles.css`, so the inferred work blocks are reachable without hunting. Frontend + `styles.css` only.
