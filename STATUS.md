...
- [x] **Live local capture panel starves the ledger work-block list** (2026-06-27) — converted `ActivityCapturePanel.tsx` from `<section>` to `<details>` disclosure (closed by default); `<summary className="section-title">` replaces the header div; `<span className="capture-panel-title">` replaces `<h2>`; added `stopPropagation` on classify button; added `summary` CSS (hide marker, cursor, title style) in `styles.css`; removed stale `max-height: 270px` constraint at line 1582; ledger list now visible at first render.
