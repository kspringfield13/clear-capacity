...
- [x] **Audit log shows raw enum/ISO correction values while Corrections screen humanizes them** — added `humanizeCorrectionValue` to the import in `App.tsx`; the `user_correction` audit summary now calls `humanizeCorrectionValue(field, value)` for both old and new values and uses "→" glyph, matching the Corrections screen display. `App.tsx`. 2026-06-25
