...
- [x] **Audit log shows raw enum/ISO correction values while Corrections screen humanizes them** — added `humanizeCorrectionValue` to the import in `App.tsx`; changed audit summary at line 514 from raw `` `${old} -> ${new}` `` to `` `${humanizeCorrectionValue(field, old)} → ${humanizeCorrectionValue(field, new)}` `` so Audit log matches Corrections screen ("Planned → Unplanned", formatted times). `App.tsx`. 2026-06-25
