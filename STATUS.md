...
### UI & UX Polish
- [ ] **Live local capture panel starves the ledger work-block list** — on the Activity Ledger the Current-block card + the `.activity-capture-panel` (`components/ledger/ActivityCapturePanel.tsx`, capped at `max-height: 270px` in `styles.css:1582`) consume nearly all the screen height, so the primary `.ledger-list` is squeezed to ~1.5 blocks at 1280×900 and to effectively ZERO at a 420px viewport (verified in the visual pass — the narrow ledger shows the capture panel then a blank void, no work blocks). Mirror the just-shipped ActivityHeatmap fix (commit 048eb12): wrap the capture panel body in a collapsible `<details className="activity-capture">` (replace the `<h2>Live local capture</h2>` header with a `<summary>`, closed by default) so the work list — the whole point of the screen — is visible at first render. `ActivityCapturePanel.tsx` + `styles.css` only.
...
- [ ] **Forecast screen shows 38% and 24% for the same metric without reconciling them** — the Forecast tab intro and accuracy banner say the deterministic reliable-capacity estimate is 38% ("the model now computes 38%" — `components/capacity/ForecastAgentPanel.tsx:82`), yet the Forecast Agent's first scenario card reads "Reliable new-work capacity 24%" / "Likely 24%" (lines ~133–146) with no note that the AI scenario refines the 38% baseline, so the two numbers read as contradictory (confirmed in the visual pass — the same "reliable new-work capacity" phrase appears as 38% in the header/sidebar and 24% in the card). Relabel/annotate the scenario card (e.g. a `<small>` "AI likely case — refined from the 38% baseline") or add a one-line reconciling note so the deterministic baseline and the AI scenario are clearly distinct. Frontend only.
