...
- [x] **Live local capture panel starves the ledger work-block list** (2026-06-27) — converted `ActivityCapturePanel.tsx` to a `<details className="activity-capture-panel">` disclosure (closed by default); replaced `<h2>Live local capture</h2>` with `<summary>`; removed the now-obsolete `max-height: 270px; overflow: hidden` CSS hack from `styles.css`; ledger work-block list now visible at first render.
