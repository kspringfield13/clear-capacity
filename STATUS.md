...
- [x] **"Current block" can highlight the wrong block** (2026-06-27) — replaced `blocks[7] ?? blocks[0]` with `blocks.reduce()` selecting the block with the latest `end_time`; falls back gracefully to `undefined` for empty arrays (already rendered conditionally). `LedgerScreen.tsx` only.
