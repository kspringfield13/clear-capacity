...
- [x] **Ledger work blocks sit below the fold under the heatmap** (2026-06-27) — converted `ActivityHeatmap.tsx` outer `<section>` to `<details>` with `<summary className="eyebrow">`; data-rich branch starts collapsed (frees space for `.ledger-list`); sparse-data branch starts `open` (keeps onboarding caption visible); added `details.activity-heatmap > summary { cursor: pointer }` in `styles.css`. Self-review confirmed and sparse-caption regression fixed before commit.
