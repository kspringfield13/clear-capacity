...
- [x] **Audit log shows raw enum/ISO correction values while Corrections screen humanizes them** — added `humanizeCorrectionValue` import to `App.tsx`; changed audit `summary` from `` `${old} -> ${new}` `` to use `humanizeCorrectionValue(field, …)` for both values with "→" glyph, matching `CorrectionsScreen.tsx` display. `App.tsx` only. 2026-06-25
