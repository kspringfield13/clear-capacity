...
### UI & UX Polish
- [ ] **BlockCard category select clips its current label** — confirmed in the visual pass: on the Daily review `.tag-grid` the category `<select>` truncates the longest `WorkCategory` ("Documentation / requirement cla…"). It has a hover `title` but the visible truncation reads as unpolished and is invisible to keyboard/touch. The `.review-screen .tag-grid` rule in `styles.css` (line ~2019) currently uses `grid-template-columns: minmax(0, 1.5fr) minmax(0, 1fr) minmax(0, 1fr)` — widen the first (category) column relative to the planned-status/mode columns (e.g. `minmax(0, 2.4fr) minmax(0, 1fr) minmax(0, 1fr)`) so the active value is readable at rest. Note: the wider `.ledger-screen .tag-grid` does NOT truncate, so scope the fix to the review variant. `BlockCard.tsx` renders the selects.
- [ ] **Onboarding checklist incomplete steps are dead-ends** — on the Setup screen "Getting started" shows 3/4 complete with "AI provider configured" rendered as a bare unchecked `<li>` (empty icon span, no affordance) — a new analyst has no way to tell *how* to finish it, and the AI-provider form lives in the collapsed `<details className="advanced-settings">` below. In `SetupScreen.tsx` (the `steps` map at lines 195–203), give each incomplete step a short muted hint describing where to act (e.g. "AI provider configured → set up in Advanced Settings", "Calendar imported → import an .ics below"), styled via a new `.onboarding-step-hint` token-based rule in `styles.css`. Pure frontend; turns the checklist from a status display into a guide.
