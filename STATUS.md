...
- [x] **Weekly "Delivery risk modifiers" bars don't encode severity by color** — added `data-severity` attribute (`low`/`mid`/`high`) to `.risk-track > span` in `RiskRow.tsx` based on value thresholds (<0.34/0.34–0.67/≥0.67); added matching CSS rules for light and dark themes (low: muted blue-gray, mid: amber, high: orange) in `styles.css`; `dangerActive` row's red treatment left untouched. 2026-06-25
