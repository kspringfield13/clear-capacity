...
- [x] **Ledger work blocks sit below the fold under the heatmap** (2026-06-27) — converted `ActivityHeatmap.tsx` `<section>` to `<details>` disclosure; the full 7-day grid starts collapsed so `.ledger-list` is immediately visible; sparse-data case starts open; added `cursor: pointer; margin: 0` to `.activity-heatmap summary` in `styles.css`.
