...
### UI & UX Polish
- [ ] **Label the bare capacity percentage** — The big bold number `pct(block.estimated_capacity_pct)` renders with no label in `BlockCard.tsx` (`.block-main strong`, daily + ledger rows) and `LedgerScreen.tsx` (`.pulse-meter strong`, current block). A new analyst can't tell "9%" means the block's share of the week. Add a small caption (e.g. "of week" / "weekly capacity") next to or under the number, and on the current-block card swap the misleading `TimerReset` clock icon for a capacity/share glyph (e.g. `PieChart`) since the value is a percentage, not a duration. Add `.block-main`/`.pulse-meter` caption tokens (light + dark) in `styles.css`.
- [ ] **Theme preference is lost on reload** — In `App.tsx`, `theme` state inits to `"light"` (line ~323) and the effect at ~line 400 immediately calls `writeThemePreference(theme)` on mount, overwriting the value `main.tsx` loaded from `readThemePreference()`. So a user's dark-mode choice reverts to light after reload. Hydrate the initial `theme` from the persisted preference on mount (e.g. `readThemePreference().then(setTheme)` plus a first-render guard so the write effect doesn't clobber it), keeping the existing `setTheme`/toolbar toggle wiring intact.
