...
- [x] **Audit log shows raw enum/ISO correction values while Corrections screen humanizes them** — added `humanizeCorrectionValue` import to `App.tsx`; changed audit `summary` from raw `old_value -> new_value` to `humanizeCorrectionValue(field, old) → humanizeCorrectionValue(field, new)` so Audit log ("Planned → Unplanned", "9:00 AM → 10:00 AM") now matches `CorrectionsScreen.tsx` display. 2026-06-25
