...
- [x] **"Current block" can highlight the wrong block** (2026-06-27) — replaced hardcoded `blocks[7] ?? blocks[0]` with a `reduce` over `end_time || start_time` to pick the block with the latest timestamp; falls back to `undefined` (card hidden) on empty array. `LedgerScreen.tsx:64` only.
