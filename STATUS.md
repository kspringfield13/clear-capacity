...
### UI & UX Polish
- [ ] **Forecast detail cards wrap 3+1, stranding "Assumptions" alone** — `.forecast-grid` in `styles.css` (line 2860) uses `repeat(auto-fit, minmax(240px, 1fr))`, so at the panel's ~975px width Constraints / Risk flags / Recommended actions fill row one and "Assumptions" drops onto a lopsided second row by itself (seen on the Forecast tab, light + dark). Switch `.forecast-grid` to a balanced two-column layout (`grid-template-columns: repeat(2, minmax(0, 1fr))`) so the four `ForecastList`s in `components/capacity/ForecastAgentPanel.tsx` (lines 136–139) read as an even 2×2 block; add a single-column fallback in the existing `@media (max-width: 600px)` rule.
- [ ] **Forecast scenarios are bare numbers with no sense of range** — the Conservative / Likely / Optimistic cards in `.forecast-summary` (`components/capacity/ForecastAgentPanel.tsx` lines 115–129) present 14% / 24% / 34% as three disconnected figures, so the spread between the protected and optimistic cases isn't legible at a glance. Add a slim horizontal range track beneath the summary row spanning conservative→optimistic with a labeled marker at "Likely", reusing the `--surface-muted` track + `--info` fill tokens (mirror `CapacityRing` / `.review-progress-track`); give it `role="img"` + an `aria-label` describing the range. `ForecastAgentPanel.tsx` + `styles.css` only.
