...
### UI & UX Polish
- [ ] **Single AI suggestion looks marooned in the Daily "Suggested cleanup" panel** — `.copilot-inline .copilot-list` uses `grid-template-columns: repeat(auto-fit, minmax(280px, 360px))` (`styles.css` line ~2139), so when the copilot returns one suggestion the 360px card sits alone against ~880px of empty panel (clearly visible on the Daily screen, light + dark — verified in this run's screenshots). Constrain the emptiness — e.g. give `.copilot-inline` (or the list) a `max-width` so the single card reads as intentional rather than broken. `ReviewCopilotPanel.tsx` (`components/review/`) / `styles.css`.
- [ ] **Weekly "Delivery risk modifiers" bars don't encode severity by color** — on the Weekly → Capacity screen the index-magnitude bars in `RiskRow.tsx` (Context switch 33, WIP overload 90, Carryover 10, Meeting density 46) all render in the same accent fill, so a 90/100 reads no more urgent than a 10/100 (clearly visible on the Weekly screen, light + dark). Add value-threshold tinting to the non-danger index bars (e.g. low <34 muted, mid 34–66 amber, high ≥67 stronger warning hue) via a `data-severity` attribute on the `.risk-track > span` fill plus matching light/dark tokens in `styles.css`. Leave the `dangerActive` "Active blockers" count row's existing `var(--danger)` red treatment untouched — the count-vs-index split is already solved; do not change it. `RiskRow.tsx` + `styles.css`.
