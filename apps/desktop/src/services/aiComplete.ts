...
export interface AiCompleteRequest {
  ...
  /** When true, Rust captures a screenshot and injects it as an image input. */
  captureScreen?: boolean;
  /** When true, fall back to OPENAI_VISION_MODEL before OPENAI_MODEL. */
  visionModelFallback?: boolean;
...
export interface AiCompleteResponse {
  ...
  rawScreenshotRetained?: boolean | null;
}
...
export async function aiComplete(request: AiCompleteRequest): Promise<AiCompleteResponse> {
  return invoke<AiCompleteResponse>("ai_complete", { request });
}
