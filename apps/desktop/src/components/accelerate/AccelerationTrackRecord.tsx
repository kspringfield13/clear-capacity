...
import { accelerationTypeLabel, formatIsoWeekLabel, realizedSavingsRatingLabel } from "../../lib/format";
      ...
      <div className="acceleration-track-record-header">
        ...
        <div>
          ...
          <p>
            ...
            about {summary.total_realized_minutes} min reclaimed against the ~
            {summary.total_projected_minutes} min they projected
        ...
        {entries.map((entry) => {
          ...
          const realizedLabel =
            entry.realized_minutes >= 0
              ? `~${entry.realized_minutes} min reclaimed`
              : `load rose ~${Math.abs(entry.realized_minutes)} min`;
          const deltaGloss =
            entry.rating === "beat"
              ? `reclaimed ~${entry.realized_minutes} min against a ~${entry.projected_minutes} min projection`
              : entry.rating === "met"
                ? `reclaimed ~${Math.max(0, entry.realized_minutes)} min, near the ~${entry.projected_minutes} min projected`
                : `reclaimed ${entry.realized_minutes >= 0 ? `~${entry.realized_minutes} min` : "nothing"}, below the ~${entry.projected_minutes} min projected`;
          return (
            <li
            ...
            >
              ...
              <span className="acceleration-track-record-values">
                <span className="acceleration-track-record-metric">
                  Projected <strong>~{entry.projected_minutes} min</strong>
                </span>
