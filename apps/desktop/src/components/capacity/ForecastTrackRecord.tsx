...
import { forecastRatingLabel, formatIsoWeekLabel, pct } from "../../lib/format";

// Audit trail for the forecast model: list how each past forecast landed against the
// capacity the model later computed for the week it targeted. Pairing/scoring happens in
// useDerived (`forecastTrackRecord`); this is presentation-only.
export function ForecastTrackRecord({ entries }: { entries: ForecastTrackRecordEntry[] }) {
    ...
    <section className="forecast-track-record" aria-label="Forecast track record">
      <div className="forecast-track-record-header">
        ...
        <div>
          ...
          <p>How past forecasts landed against the reliable capacity the model later computed for each week.</p>
        </div>
      </div>
