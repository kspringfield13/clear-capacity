...
// ---------------------------------------------------------------------------
// Agent Skills (SKILL.md) export — turn a saved recipe into a runnable skill in
// Anthropic's SKILL.md open standard (agentskills.io). Everything here is pure
// string building over already-derived fields (title/detail/recipe/tools) — no
// window titles, no network — so exporting to a local file is privacy-safe.
// ---------------------------------------------------------------------------
...
export function serializeSavedSkillAsSkillMd(skill: SavedSkill): string {
  ...
  lines.push(
    "---",
    `_Generated by ClearCapacity from observed work — estimated ~${skill.estimated_minutes_saved_per_week} min saved per week. Review before running._`,
    ""
  );
  return lines.join("\n");
}
 ...
 * names its target path — split each into `.claude/skills/<name>/SKILL.md` to install.
 */
export function serializeSavedSkillsAsSkillBundle(skills: SavedSkill[], now = new Date()): string {
  const header = [
    "# ClearCapacity — Agent Skills export",
    "",
    `${skills.length} skill${skills.length === 1 ? "" : "s"} exported ${now.toISOString()}.`,
    "Each section below is a complete SKILL.md (Anthropic Agent Skills standard). To install one,",
    "create a folder `.claude/skills/<name>/` and save its content as `SKILL.md` there, or drop it",
  ...
  const sections = skills.map((skill) =>
    [
      "",
      "==================================================================",
      `FILE: .claude/skills/${skillName(skill)}/SKILL.md`,
      "==================================================================",
      "",
